...
```{r, message=FALSE}
install.packages("ggplot2") # makes for easier filepath management
...
```{r, message=FALSE}
names(raw_penguins_data_from_package)[names(raw_penguins_data_from_package) == "Culmen Length (mm)"] <- "culmen_length_mm"
```
...
```{r, message=FALSE}
skimr::skim(raw_penguins_data_from_package)
#summary(raw_penguins_data_from_package)
```
...
-   Let's check out `DataExplorer`
    ...
    introduce(raw_penguins_data_from_package)
    plot_intro(raw_penguins_data_from_package)
    ```
...
```{r, message=FALSE}
x=ggplot(data = penguins_data, aes(x = culmen_depth_mm, color = species)) +
  geom_histogram() +
  theme_bw() + 
  scale_fill_viridis_d(option="A") + 
  labs(title = "Palmer Penguins, Distribution of culmen depth",
